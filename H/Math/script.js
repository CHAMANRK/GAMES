'use strict';
/* ═══════════════════════════════════════════════
   ALL-IN-ONE AI QUIZ — script.js  v3.0
   6-Screen Flow: Name → Category → Config → Loading → Game → Result
═══════════════════════════════════════════════ */
...
RULES:
- Answer = short word/name (e.g. "Rahul", "Teen", "Haan")
...
Examples: "Subah char paon dopahar do paon shaam teen paon" → "Insaan"
"Mujhe todte hain toh kaam aata hoon todte nahi toh bekaar" → "Anda"
"Jitna kheencho utna lambi wapas chodne par chhoti" → "Rubber band"
RULES:
- Answer = 1-3 words ONLY
- No multiple choice
JSON only: {"question":"...","answer":"...","explanation":"brief explanation"}`,

    rishte:
`You are an expert at Indian blood relation puzzles. Generate ONE verified puzzle.
Difficulty: ${diff}
${lang}
VERIFIED CORRECT EXAMPLES (copy this style exactly):
Easy — Q:"Aapke papa ki maa aapki kaun hoti hain?" A:"Dadi"
Easy — Q:"Aapki maa ke bhai aapke kaun hote hain?" A:"Mama"
Medium — Q:"Aapke mama ki ikloti behen aapki kaun hoti hain?" A:"Maa"
Medium — Q:"Aapke chacha ke bete aapke kaun hote hain?" A:"Cousin"
Hard — Q:"Aapke nana ke damad aapke kaun hote hain?" A:"Papa"

STRICT RULES:
- Easy=1 step | Medium=2 steps | Hard=3 steps MAX (no longer chains)
- Answer = ONE relation word: Dadi/Nana/Mama/Chacha/Papa/Maa/Bhai/Behen/Cousin/Bhanji/Bhatija/Saas/Sasur
- BEFORE writing: mentally trace the chain step by step and verify answer is correct
- If you are not 100% sure, pick an easier question
JSON only: {"question":"...","answer":"...","explanation":"Step 1: X → Step 2: Y → Answer: Z"}`,

    gk:
`Generate a mind-blowing GK/science question for an Indian quiz game.
Difficulty: ${diff}
${lang}
VERIFIED EXAMPLES (factually correct — use same style):
...
RULES:
- Answer = FACTUALLY 100% CORRECT, 1-4 words
- For number answers use format: "1 baar", "365 din", "7" etc.
- NO trick questions with debatable answers
- Explanation = clear factual, satisfying
...
async function fetchQuestion() {
  const data = await callProxy(
    [
      { role:'system', content:'You are a quiz question generator. Respond ONLY in pure valid JSON. No markdown, no extra text, no explanation outside JSON.' },
      { role:'user',   content: buildQuestionPrompt() }
    ],
    { temperature:0.82, max_tokens:400, responseFormat:'json' }
  );
...
function normalize(s) {
  return String(s).trim().toLowerCase()
    .replace(/baar/g,'bar').replace(/بار/g,'bar')
    .replace(/\bek\b/g,'1').replace(/\bdo\b/g,'2').replace(/\bteen\b/g,'3')
    .replace(/\s+/g,' ').replace(/[।,.'"\-]/g,'')
    .replace(/rs\.?/gi,'').replace(/×/g,'*').replace(/÷/g,'/');
}
function normalizeCompact(s){ return normalize(s).replace(/\s+/g,''); }
function parseMath(s) {
  const c=s.replace(/\s/g,'');
  if(c.includes('/')) { const p=c.split('/'); if(p.length===2) return parseFloat(p[0])/parseFloat(p[1]); }
  return parseFloat(c);
}

function checkAnswer(userRaw,rightRaw) {
  const u=normalize(userRaw), r=normalize(rightRaw);
  const uc=normalizeCompact(userRaw), rc=normalizeCompact(rightRaw);
  if(u===r||uc===rc) return true;
  if(rc.includes(uc)&&uc.length>=2) return true;
  if(uc.includes(rc)&&rc.length>=2) return true;
  const uw=u.split(' ').filter(w=>w.length>=2);
  const rw=r.split(' ').filter(w=>w.length>=2);
  if(uw.length>0&&uw.every(w=>r.includes(w))) return true;
  if(rw.length>0&&rw.every(w=>u.includes(w))) return true;
  const un=parseMath(uc), rn=parseMath(rc);
