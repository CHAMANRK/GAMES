...
import { getAuth, GoogleAuthProvider, GoogleAuthProvider as GP, signInWithPopup, signOut, onAuthStateChanged, RecaptchaVerifier, signInWithPhoneNumber, PhoneAuthProvider, linkWithCredential, EmailAuthProvider, createUserWithEmailAndPassword, signInWithEmailAndPassword, reauthenticateWithPopup } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js";
...
let setPinStep   = 1;
let setPinFirst  = '';
let setPinEntry  = ''; // BUG 7 FIX: single declaration here (was declared twice causing hoisting conflict)
let verifyPinEntry = '';
...
//  AUTH
// ════════════════════════════
// Safety fallback — 12 sec ke baad bhi loading hai toh auth screen dikhao
...
window.showAuthOptions = () => {
    ...
    // BUG 1 FIX: Clear recaptcha when going back, so it can re-render fresh next time
    if (window.loginRecaptcha) {
        try { window.loginRecaptcha.clear(); } catch(e) {}
        window.loginRecaptcha = null;
    }
};
...
window.showPhoneLogin = async () => {
    document.getElementById('auth-login-options').classList.add('hidden');
    document.getElementById('auth-phone-form').classList.remove('hidden');
    // BUG 1+2 FIX: Always create fresh instance and properly await render
    try {
        if (!window.loginRecaptcha) {
            window.loginRecaptcha = new RecaptchaVerifier(auth, 'phone-login-recaptcha', {
                size: 'normal',
                callback: () => {}
            });
            await window.loginRecaptcha.render(); // BUG 2 FIX: await was missing!
        }
    } catch(e) {
...
window.loginEmail = async (isRegister) => {
    const email = document.getElementById('email-input').value.trim();
    const pass  = document.getElementById('password-input').value;
    if (!email || !pass) { window.showToast('Email aur password daalo', 'error'); return; }
    try {
        show('loading-screen');
        if (isRegister) {
...
window.sendLoginOTP = async () => {
    const num = document.getElementById('phone-login-input').value.trim();
    if (num.length !== 10) { window.showToast('10-digit number daalo', 'error'); return; }
    try {
        if (!window.loginRecaptcha) {
            window.loginRecaptcha = new RecaptchaVerifier(auth, 'phone-login-recaptcha', { size: 'normal', callback: () => {} });
            await window.loginRecaptcha.render();
        }
        loginConfirmResult = await signInWithPhoneNumber(auth, '+91' + num, window.loginRecaptcha);
        document.getElementById('phone-login-otp-wrap').classList.remove('hidden');
        window.showToast('📱 OTP bheja gaya!');
    } catch(e) {
        console.error('sendLoginOTP:', e);
        window.showToast('❌ ' + e.message, 'error');
        // BUG 2 FIX: Safely clear recaptcha on error
        if (window.loginRecaptcha) { try { window.loginRecaptcha.clear(); } catch(_) {} window.loginRecaptcha = null; }
    }
};
window.verifyLoginOTP = async () => {
    const otp = document.getElementById('phone-login-otp').value.trim();
    if (!otp || otp.length < 6) { window.showToast('6-digit OTP daalo', 'error'); return; }
    // BUG 5 FIX: null check on loginConfirmResult
    if (!loginConfirmResult) { window.showToast('Pehle OTP bhejo!', 'error'); return; }
    try {
        show('loading-screen');
        await loginConfirmResult.confirm(otp);
    } catch(e) {
        hide('loading-screen');
        window.showToast('❌ Galat OTP! Dobara try karo.', 'error');
    }
};
...
function handleUserLogin(user) {
    ...
    const pin = getPin();
    if (!pin) {
        // New user: check phone first, then set PIN
        checkPhoneThenPin(user);
    } else {
...
function checkPhoneThenPin(user) {
    // Agar phone number already linked hai (phone login ya previously linked)
    const hasPhone = user.phoneNumber || localStorage.getItem(`mm_phone_${user.uid}`);
    const skipUntil = localStorage.getItem(`mm_skip_phone_${user.uid}`);
    const skipped = skipUntil && Date.now() < parseInt(skipUntil);

    if (hasPhone || skipped) {
        showSetPinScreen();
    } else {
        // Phone link screen dikhao
        showPhoneLinkScreen();
    }
}

// ─── PHONE LINK SCREEN ───
function showPhoneLinkScreen() {
    hide('auth-screen');
    hide('loading-screen');
    show('phone-link-screen');
    document.getElementById('link-phone-input').value = '';
    document.getElementById('link-otp-wrap').classList.add('hidden');
    document.getElementById('link-phone-error').classList.add('hidden');
}

let linkConfirmResult = null;
window.sendLinkOTP = async () => {
    const num = document.getElementById('link-phone-input').value.trim();
    if (num.length !== 10) { window.showToast('Sahi 10-digit number daalo', 'error'); return; }
    const fullNum = '+91' + num;
    try {
        if (!window.linkRecaptcha) {
            window.linkRecaptcha = new RecaptchaVerifier(auth, 'link-phone-recaptcha', {
                size: 'invisible', callback: () => {}
            });
            await window.linkRecaptcha.render();
        }
        linkConfirmResult = await signInWithPhoneNumber(auth, fullNum, window.linkRecaptcha);
        document.getElementById('link-phone-preview').textContent = num;
        document.getElementById('link-otp-wrap').classList.remove('hidden');
        window.showToast('📱 OTP bheja gaya +91' + num);
    } catch(e) {
        console.error('sendLinkOTP:', e);
        window.showToast('❌ ' + e.message, 'error');
        document.getElementById('link-phone-error').classList.remove('hidden');
        // BUG 4 FIX: safely clear linkRecaptcha on error
        if (window.linkRecaptcha) { try { window.linkRecaptcha.clear(); } catch(_) {} window.linkRecaptcha = null; }
    }
};

window.verifyLinkOTP = async () => {
    const otp = document.getElementById('link-otp-input').value.trim();
    if (!otp || otp.length < 6) { window.showToast('6-digit OTP daalo', 'error'); return; }
    // BUG 5 FIX: null check — prevent crash if OTP was never sent
    if (!linkConfirmResult) { window.showToast('Pehle OTP bhejo!', 'error'); return; }
    try {
        const result = await linkConfirmResult.confirm(otp);
        // Phone linked — save it
        const phone = '+91' + document.getElementById('link-phone-input').value.trim();
        localStorage.setItem(`mm_phone_${currentUser.uid}`, phone);
        hide('phone-link-screen');
        window.showToast('✅ Phone linked! Ab PIN set karo');
        showSetPinScreen();
    } catch(e) {
        document.getElementById('link-phone-error').classList.remove('hidden');
        window.showToast('❌ Galat OTP!', 'error');
    }
};

window.skipPhoneLink = () => {
    // 3 din ke liye skip
    const skipUntil = Date.now() + 3 * 24 * 60 * 60 * 1000;
    localStorage.setItem(`mm_skip_phone_${currentUser.uid}`, skipUntil);
    hide('phone-link-screen');
    window.showToast('⚠️ Phone link karna mat bhoolo!', 'error');
    showSetPinScreen();
};
...
window.forgotPin = async () => {
    // Check karo phone linked hai ya nahi
    const phone = currentUser.phoneNumber || localStorage.getItem(`mm_phone_${currentUser.uid}`);
    if (!phone) {
        window.showToast('❌ Phone number link nahi hai! PIN reset possible nahi.', 'error');
        return;
    }
    show('forgot-pin-screen');
    hide('lock-screen');
    document.getElementById('forgot-pin-sub').textContent = `OTP bheja ja raha hai ${phone} pe...`;
    document.getElementById('forgot-otp-wrap').classList.add('hidden');
    document.getElementById('forgot-otp-error').classList.add('hidden');
    await window.sendForgotOTP(phone);
};

let forgotConfirmResult = null;
window.sendForgotOTP = async (phone) => {
    try {
        if (!window.forgotRecaptcha) {
            window.forgotRecaptcha = new RecaptchaVerifier(auth, 'forgot-pin-recaptcha', {
                size: 'invisible',
                callback: () => {}
            });
            await window.forgotRecaptcha.render();
        }
        forgotConfirmResult = await signInWithPhoneNumber(auth, phone, window.forgotRecaptcha);
        document.getElementById('forgot-pin-sub').textContent = `OTP bheja: ${phone}`;
        document.getElementById('forgot-otp-wrap').classList.remove('hidden');
        window.showToast('📱 OTP bheja gaya!');
    } catch(e) {
        console.error('sendForgotOTP:', e);
        document.getElementById('forgot-pin-sub').textContent = '❌ OTP nahi gaya. Dobara try karo.';
        window.showToast('❌ ' + e.message, 'error');
        // BUG 6 FIX: safely clear forgotRecaptcha on error
        if (window.forgotRecaptcha) { try { window.forgotRecaptcha.clear(); } catch(_) {} window.forgotRecaptcha = null; }
    }
};
...
window.verifyForgotOTP = async () => {
    ...
    if (!otp || otp.length < 6) { window.showToast('6-digit OTP daalo', 'error'); return; }
    // BUG 6 FIX: null check — crash prevent
    if (!forgotConfirmResult) { window.showToast('OTP pehle bheja nahi gaya!', 'error'); return; }
    try {
        await forgotConfirmResult.confirm(otp);
        // OTP verified — same user re-authenticated
        hide('forgot-pin-screen');
        localStorage.removeItem(`mm_pin_${currentUser.uid}`);
        window.showToast('✅ Verified! Naya PIN set karo');
        showSetPinScreen();
    } catch(e) {
        document.getElementById('forgot-otp-error').classList.remove('hidden');
        document.getElementById('forgot-otp-input').value = '';
        setTimeout(() => document.getElementById('forgot-otp-error').classList.add('hidden'), 2000);
    }
};

window.cancelForgotPin = () => {
    hide('forgot-pin-screen');
    show('lock-screen');
};
...
// BUG 7 FIX: setPinEntry already declared at top — removed duplicate `let` here
