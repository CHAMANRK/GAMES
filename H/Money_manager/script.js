...
window.showPhoneLogin = () => {
    ...
    if (!window.loginRecaptcha) {
        window.loginRecaptcha = new RecaptchaVerifier(auth, 'phone-login-recaptcha', {
            size: 'normal',
            callback: () => { document.querySelector('.btn-phone')?.removeAttribute('disabled'); }
        });
        window.loginRecaptcha.render();
    }
};
...
window.sendLoginOTP = async () => {
    ...
    try {
        ...
        window.showToast('OTP failed: ' + e.message, 'error');
    }
};
...
window.sendForgotOTP = async (phone) => {
    try {
        if (!window.forgotRecaptcha) {
            window.forgotRecaptcha = new RecaptchaVerifier(auth, 'forgot-pin-screen', {
                size: 'invisible',
                callback: () => {}
            });
        }
