...
import { isValidParaRange }  from './helpers.js';

// switchTab — lazy import to avoid circular dependency (quiz → auth → online-match → quiz)
async function switchToSignup() {
  const { switchTab } = await import('./auth.js');
  switchTab('signup');
}
...
export async function startGame() {
  ...
  if (curUser?.isAnonymous && getGuestAnswered() >= CONFIG.GUEST_QUESTION_LIMIT) {
    ...
    else { showScreen('authScreen'); switchToSignup(); }
    return;
  }
...
export function checkAnswer() {
  ...
  if (ok || quizState.mode !== 'survival') {
    incGuestAnswered();
    // Guest limit check — modal dikhao
    const curUser2 = getCurUser();
    if (curUser2?.isAnonymous && getGuestAnswered() >= CONFIG.GUEST_QUESTION_LIMIT) {
      timerManager.clearInterval('autoNext');
      const gm = $('guestModal');
      if (gm) gm.style.display = 'flex';
      return;
