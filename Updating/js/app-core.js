...
export function initAuthListener() {
  ...
  // ✅ FIXED: Increased failsafe timeout from 8000 to 12000
...
export function startFriendsListener() {
  ...
  const unsub = onSnapshot(doc(db,'users',curUser.uid), async snap => {
    ...
    fl.innerHTML = '';
    if (!friends.length) { fl.innerHTML='<div style="color:var(--text-muted);font-size:.88rem;text-align:center;padding:16px;font-family:Tajawal,sans-serif">Koi dost nahi — UID se dhundein!</div>'; return; }
