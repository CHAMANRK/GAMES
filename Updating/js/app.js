...
async function syncUser(uid, data) {
  try {
    ...
    // Start/refresh real-time listener (listenToUserData handles dedup)
...
// Central real-time listener — replaces scattered getDoc + updateHeader calls
function listenToUserData(uid) {
  if (_userDataUnsub) { _userDataUnsub(); _userDataUnsub = null; }
  const r2 = doc(db, 'users', uid);
  _userDataUnsub = onSnapshot(r2, snap => {
    if (snap.exists()) {
      curData = snap.data();
      updateHeader();
      checkUnlocks();
    }
  }, err => console.warn('listenToUserData error:', err.message));
}
...
onAuthStateChanged(auth, async user => {
  ...
  if (user) {
    if (!user.isAnonymous) {
      // Ensure doc exists (first time), then start real-time listener
      try {
...
let matchState = {
  ...
  listeners: [],
  feeDeducted: false, autoCancel: null
};

function openOnlineLobby() {
  if (!curUser || curUser.isAnonymous) { toast('❌ Login karo pehle!', 'error'); showScreen('authScreen'); return; }
  loadQuran();  // Pre-load in background so it's ready when match starts
  const coins = curData?.coins || 0;
...
onValue(ref(rtdb, '.info/connected'), snap => {
  ...
  if (!_isConnected) console.warn('⚠️ RTDB: Offline');
});
...
async function joinTable(tableKey) {
  ...
  // Deduct fee — will be refunded if no opponent found
  ...
  matchState.feeDeducted = true;  // track for refund

  showScreen('matchWaitScreen');
  $('matchWaitInfo').textContent = table.name;

  // Wait timer using Date.now() — immune to background tab throttling
...
async function findOrCreateMatch(tableKey) {
  ...
  // ── ATOMIC queue check using runTransaction ──
  // Prevents race condition: two players both seeing empty queue simultaneously
  try {
    await runTransaction(queueRef, (currentData) => {
      if (currentData === null) {
        // Queue is empty — we become P1, write ourselves in
        return {
          matchId:  null,  // will be set after we create the match node
          uid:      curUser.uid,
        ...
        // Someone is waiting — we are P2, take their slot (return undefined = abort & keep data)
        joinedAsP2    = true;
        matchIdToJoin = currentData.matchId;
        opponentData  = currentData;
        return; // abort transaction, queue stays so we can read it
      }
    });
  } catch(e) {
  ...
  if (joinedAsP2 && matchIdToJoin) {
    // ── P2 path ──
    await remove(queueRef);  // Clear queue entry
    matchState.matchId      = matchIdToJoin;
    matchState.myRole       = 'p2';
    matchState.opponentName = opponentData.username;
    await joinAsP2(matchIdToJoin, tableKey);

  } else {
    // ── P1 path ── Create match node, then update queue with matchId
    ...
    // Update queue entry with the new matchId
    await update(queueRef, { matchId });

    // Listen for p2 joining
    waitForOpponent(tableKey, matchId);

    // Auto-cancel after 60s if no opponent (refund coins)
...
async function refundFee(tableKey) {
  if (!matchState.feeDeducted || !curUser || !curData) return;
  const fee = TABLES[tableKey]?.fee || 0;
  if (fee <= 0) return;
  matchState.feeDeducted = false;
  curData.coins = (curData.coins || 0) + fee;
  updateHeader();
...
async function joinAsP2(matchId, tableKey) {
  await loadQuran();  // Ensure quran data is ready
  const matchRef = ref(rtdb, `matches/${tableKey}/${matchId}`);
...
function startOnlineMatch(tableKey, matchId, initialData) {
  ...
  matchState.feeDeducted = false;  // Match started — no refund on exit
...
function leaveMatchCleanup(cancelRefund = false) {
  clearInterval(matchState.waitTimer);
  clearInterval(matchState.matchTimerInterval);
  clearTimeout(matchState.autoCancel);
  matchState.autoCancel = null;
  ...
  // Refund coins ONLY if match never started (still waiting for opponent)
  if (cancelRefund && matchState.feeDeducted && matchState.tableKey && matchState.myRole === 'p1') {
    refundFee(matchState.tableKey);
    toast('↩️ Cancel — coins wapas mil gaye!', 'info', 3000);
  }
...
// ═══════════════════════════════════════════
//  START — Quran loads lazily on first game/match
// ═══════════════════════════════════════════
