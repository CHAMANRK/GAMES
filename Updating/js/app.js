...
let _autoNextTmr = null; // auto-next timer
...
function refreshProfilePanel() {
  ...
  const uname   = curData.username || curUser.displayName || 'Player';
  const role    = isHafiz ? 'Hafiz' : (curData.role === 'admin' ? 'Admin' : 'Player');

  const set = (id, val) => { const el = $(id); if (el) el.textContent = val; };
  set('ppUsername', uname);
  set('ppRole',     role);
  set('ppCoins',    (curData.coins || 0).toLocaleString());
  set('ppAccuracy', (curData.accuracy || 0) + '%');
  set('ppGames',    curData.totalGames || 0);
...
function updateHeader() {
  ...
  if (curUser && !curUser.isAnonymous) {
    // Registered user — show profile btn + coins
...
// ═══════════════════════════════════════════
function nextQ() {
  // Clear auto-next timer if user clicked manually
  if (_autoNextTmr) { clearInterval(_autoNextTmr); _autoNextTmr = null; }
  const _nb2 = $('nextBtn');
  if (_nb2) _nb2.textContent = '➡️ Agla Sawal';
...
function checkAnswer() {
  ...
  // Auto-advance: 5 second baad automatically next question
  if (_autoNextTmr) clearInterval(_autoNextTmr);
  let _cd = 5;
  const _nb = $('nextBtn');
  if (_nb) _nb.textContent = `➡️ Agla Sawal (${_cd}s)`;
  _autoNextTmr = setInterval(() => {
    _cd--;
    if (_nb) _nb.textContent = _cd > 0 ? `➡️ Agla Sawal (${_cd}s)` : '➡️ Agla Sawal';
    if (_cd <= 0) { clearInterval(_autoNextTmr); _autoNextTmr = null; nextQ(); }
  }, 1000);
}
