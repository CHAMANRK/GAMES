...
function refreshProfilePanel() {
  ...
  set('ppUsername', curData.username || curUser.displayName || 'Player');
  set('ppRole',     isHafiz ? 'Hafiz' : (curData.role === 'admin' ? 'Admin' : 'Player'));
...
function nextQ() {
  ...
  const _nb = $('nextBtn');
  if (_nb) _nb.textContent = '➡️ Agla Sawal';
...
function checkAnswer() {
  // Lock: 1 question = 1 check only
  const cb = $('checkBtn');
  if (cb && cb.disabled) return; // already submitted this question
  ...
  // Auto-next: 5 second mein next question automatic
  let _cd = 5;
  const _nb = $('nextBtn');
  if (_nb) _nb.textContent = '➡️ Agla Sawal (5s)';
