...
function nextQ() {
  ...
  const nb = $('nextBtn');
  if (nb) nb.textContent = '➡️ Agla Sawal';
...
function checkAnswer() {
  // Lock check button — 1 question 1 check
  const cb = $('checkBtn');
  if (!cb || cb.disabled) return; // already submitted
  cb.disabled = true;

  if (_autoNextTmr) { clearTimeout(_autoNextTmr); _autoNextTmr = null; }
  ...
  if (!para) {
    ...
    if (cb) cb.disabled = false; // re-enable on validation error
    return;
  }
  ...
  // Auto next: 5 seconds baad automatically next question (survival game-over ke baad nahi)
  if (mode !== 'survival' || survOn) {
    let countdown = 5;
    const nb = $('nextBtn');
    if (nb) nb.textContent = `➡️ Agla Sawal (${countdown}s)`;
    _autoNextTmr = setInterval(() => {
      countdown--;
      if (nb) nb.textContent = countdown > 0 ? `➡️ Agla Sawal (${countdown}s)` : `➡️ Agla Sawal`;
      if (countdown <= 0) {
        clearInterval(_autoNextTmr);
        _autoNextTmr = null;
        if (nb) nb.textContent = '➡️ Agla Sawal';
        nextQ();
      }
