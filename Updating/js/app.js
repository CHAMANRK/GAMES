...
onAuthStateChanged(auth, async user => {
  ...
  if (user) {
    ...
    updateHeader();
    showScreen('welcomeScreen');
    // FIX: pehle _authResolved check karo, phir curData check karo
    // Pehle hideBootLoader ke baad _authResolved true hota hai
    // toh popup condition seedha curData pe depend karti thi jo async load hoti hai
    // Ab: agar curData abhi available hai toh seedha dikhao,
    // warna userListener ke pehle update par dikhega (updateHeader mein)
    if (!_authResolved && curData) {
      ...
      // curData load nahi hua abhi — thodi der baad check karo
      setTimeout(() => {
        if (curData) showWelcomePopup(curData.username||'Player', curData.coins||0);
      }, 1500);
    }
  ...
  clearTimeout(_bootFailsafe); // Failsafe cancel karo — ab zaroorat nahi
  hideBootLoader();
});
...
function showHint(){if(hints>=MAX_H)return;hints++;const hi=$('hintInfo');if(hi)hi.textContent=`Hint: ${hints}/${MAX_H}`;const hb=$('hintBtn');if(hb&&hints>=MAX_H)hb.disabled=true;const ap=curAyat.para||(((parseInt(curAyat.page)-1)/20|0)+1); // FIX: JSON paraconst s2=curAyat.surah_name.split(' ').slice(0,2).join(' ');const e=$('quizError');if(!e)return;e.innerHTML=`💡 <b>Hint (-5🪙):</b> Surah: <b>${s2}...</b>, Para: <b>${ap}</b>`;e.classList.remove('hidden');setTimeout(()=>e.classList.add('hidden'),3500);}
function endQuiz(){
