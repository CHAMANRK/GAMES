...
function applyLang(lang) {
  ...
  if (lang === 'hinglish') {
    set('txt-lang-title', 'Language');
    set('txt-lang-desc',  'Hinglish ya English');
    set('txt-logout',     'Logout');
  } else {
    set('txt-lang-title', 'Language');
    set('txt-lang-desc',  'Hinglish or English');
    set('txt-logout',     'Logout');
  }
...
function updateProfilePanel() {
  ...
  const games   = curData?.totalGames || 0;
  ...
  setT('ppUsername', (isHafiz ? '👑 ' : '') + uname);
  setT('ppRole',     isHafiz ? 'Hafiz' : (curData?.role === 'admin' ? 'Admin' : 'Player'));
  setT('ppCoins',    coins.toLocaleString());
  setT('ppAccuracy', acc + '%');
  setT('ppGames',    games);
  if ($('ppAvatarCircle')) $('ppAvatarCircle').textContent = isHafiz ? '👑' : '👤';
  if ($('profileBtnIcon')) $('profileBtnIcon').textContent = isHafiz ? '👑' : '👤';
}
...
function updateHeader() {
  ...
  const coinsEl    = $('hdrCoins');
  const coinsVal   = $('hdrCoinsVal');

  if (curUser && !curUser.isAnonymous) {
    const coins = curData?.coins || 0;
    if (profileBtn) profileBtn.classList.remove('hidden');
    if (coinsEl)    coinsEl.classList.remove('hidden');
    if (coinsVal)   coinsVal.textContent = coins.toLocaleString();
    updateProfilePanel();
  } else {
    if (profileBtn) profileBtn.classList.add('hidden');
    if (coinsEl)    coinsEl.classList.add('hidden');
  }
}
