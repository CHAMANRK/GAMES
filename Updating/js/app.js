...
function startOnlineMatch(tableKey, matchId, initialData) {
  ...
  // Dono P1 aur P2 ke liye pehla sawaal foran load karo
  ...
  const matchRef = ref(rtdb, `matches/${tableKey}/${matchId}`);
  let lastLoadedQ = initialData.currentQ || 1;
  ...
  const unsub = onValue(matchRef, snap => {
    ...
    // Naya sawaal tab hi load karo jab currentQ badhe
    if (data.ayatIndex >= 0 && data.currentQ > 0 && data.currentQ !== lastLoadedQ) {
      lastLoadedQ = data.currentQ;
      matchState.matchQ = data.currentQ;
      loadMatchQuestion(data.ayatIndex, data.currentQ, table.totalQ);
    }
  }, err => console.warn('Match listener error:', err.message));
...
async function loadFriendsList() {
  ...
  const pendLabel = document.getElementById('pendingLabel');
  if (!listEl) return;
  ...
  try {
    ...
    const pendingIn  = data.friendRequests || [];
      ...
      listEl.innerHTML = fDocs.map((fd, i) => {
        ...
        return `<div class="friend-card">
          ...
          <div class="friend-info">
            ...
            <div class="friend-uid">🆔 ${(d.uid||'').substring(0,14)}...</div>
          </div>
          <button class="friend-action-btn unfriend-btn" onclick="unfriendUser('${friends[i]}','${esc(d.username||'Player')}')">🗑️ Unfriend</button>
        </div>`;
      }).filter(Boolean).join('') || '<div style="color:var(--text-muted);text-align:center;padding:12px">Koi dost nahi</div>';
    }
    ...
    if (!pendingIn.length) {
      ...
      const pDocs = await Promise.all(pendingIn.map(uid => getDoc(doc(db, 'users', uid)).catch(() => null)));
      if (pendEl) {
        pendEl.innerHTML = pDocs.map((pd, i) => {
          ...
          return `<div class="friend-card">
            ...
            <div class="friend-info">
              <div class="friend-name">${esc(d.username || 'Player')}</div>
              <div class="friend-uid" style="color:var(--gold)">📩 Friend request</div>
            </div>
            <div style="display:flex;gap:6px">
              <button class="friend-action-btn accept-btn" onclick="acceptFriend('${pendingIn[i]}','${esc(d.username||'Player')}')">✅</button>
    ...
    listEl.innerHTML = `<div style="color:#ff9090;font-size:0.85rem;text-align:center;padding:16px">Error: ${e.message}</div>`;
  }
}
...
async function addFriend() {
  ...
  if (uid === curUser.uid) { setMsg('addFriendMsg', '❌ Apni khud ki UID nahi!'); return; }

  setMsg('addFriendMsg', '⏳ Dhoondh raha hoon...', 'success');

  try {
    // Direct UID se Firestore document fetch karo
    ...
    if (mySnap.exists()) {
      ...
      if (myFriends.includes(uid)) {
        setMsg('addFriendMsg', `✅ ${esc(targetData.username||'Player')} pehle se dost hain!`, 'success');
        return;
      }
    }

    // Check already sent request
    const theirRequests = targetData.friendRequests || [];
    if (theirRequests.includes(curUser.uid)) {
      setMsg('addFriendMsg', '⏳ Request pehle se bhej di hai!', 'success');
      return;
    }
    ...
    setMsg('addFriendMsg', `✅ Friend request bhej di: ${esc(targetData.username||'Player')}!`, 'success');
    input.value = '';
  } catch(e) {
...
// Friends event bindings
on('goFriends',       'click', openFriendsScreen);
on('backFromFriends', 'click', () => showScreen('welcomeScreen'));
on('addFriendBtn',    'click', addFriend);
const fInput = document.getElementById('friendUidInput');
if (fInput) fInput.addEventListener('keydown', e => { if (e.key === 'Enter') addFriend(); });
