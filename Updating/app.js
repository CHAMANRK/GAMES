...
function nextQ() {
  ...
  // Dynamic maxPip based on current ayat's para
...
function startOnlineMatch(tableKey, matchId, initData) {
  ...
  const unsub = onValue(mRef, snap => {
    ...
    if (oppConnected === false && !MS.disconnectGraceTimer) {
      // FIX: Grace period 5 sec — countdown dikhao, phir auto-win declare karo
      // Coins bhi milenge aur RTDB mein winner likh denge taake dusra side bhi jaane
      const GRACE_SEC = 5;
...
on('exitMatchConfirm','click', async () => {
  hideExitConfirm();
  // FIX: Pehle RTDB mein apna connected:false likho
  // Isse opponent ka disconnect listener trigger hoga aur use 5 sec baad auto-win milegi
  if (MS.matchId && MS.tableKey && MS.myRole) {
    try {
      await set(ref(rtdb, `matches/${MS.tableKey}/${MS.matchId}/${MS.myRole}/connected`), false);
    } catch(e) {}
  }
  leaveMatchCleanup(false);
  showScreen('welcomeScreen');
