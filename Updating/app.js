...
async function startGame() {
  ...
  selAyats=quranData.filter(a=>{return a.para>=fp&&a.para<=tp;}); // JSON pre-calculated para use karo (formula galat tha)
...
function checkAnswer() {
  ...
  const pn=parseInt(curAyat.page), ap=curAyat.para, aip=curAyat.pip; // JSON pre-calculated values use karo
  let parts=[],opt=0;
...
function showHint() {
  ...
  const ap = curAyat.para; // JSON pre-calculated para
...
function doSearch(e){
  ...
  const found=quranData.filter(a=>{
    ...
    if(String(a.para)===raw.trim()) return true;
    return false;
  }).slice(0,30);
...
async function submitMatchAnswer(timeOut=false) {
  ...
  const ap   = ayat.para;   // JSON pre-calculated para
  const aip  = ayat.pip;    // JSON pre-calculated pip
