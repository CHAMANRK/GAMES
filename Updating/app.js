...
async function joinTable(tableKey) {
  ...
  const coins = curData?.coins || 0;
  if (coins < table.fee) { toast(`❌ ${table.fee}🪙 chahiye! Aapke paas: ${coins}🪙`,'error'); return; }
...
function leaveMatchCleanup(refund=false) {
  ...
  MS.autoCancel = null;
  MS.disconnectGraceTimer = null;
  MS.listeners.forEach(l=>{ try{ if(l&&l.ref) { if(l.fn) off(l.ref,l.fn); else off(l.ref); } }catch(e){} });
  MS.listeners = [];
  if (MS.tableKey) {
