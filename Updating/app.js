...
//
//  New Fixes (v2):
//  [BUG-7]  COIN_RATE_LIMIT_MAX 5→20: fast players ka coins silently drop hona band
//  [BUG-8]  exitMatchConfirm: RTDB match document delete — permanent memory leak band
//  [BUG-9]  Win coins: RTDB verify karne ke baad add — client-side exploit band
//  [BUG-10] Match games bhi totalGames mein count — stats accurate huye
//  [BUG-11] feeDeducted startOnlineMatch mein clear nahi hoti — crash refund fix
//  [BUG-12] Streak bonus direct updateDoc — rate limiter bypass, bonus never drops
//  [BUG-13] doSignup welcome popup DB coins ke baad — optimistic mismatch fix
//  [BUG-14] sessionTotal double count (timer+submit) — _timedOut flag se guard
// ═══════════════════════════════════════════════════════════════
'use strict';
...
const CONFIG = {
  ...
  COIN_RATE_LIMIT_MAX:      20,   // [BUG-1 FIX] 5→20: fast players ka coins drop hona band
...
async function doSignup() {
  ...
  try {
    ...
    // [BUG-9 FIX] syncUser ke baad actual DB coins read karo — optimistic 500 hata diya
    const _d = getCurData(); const _coins = _d?.coins ?? 500;
    showScreen('welcomeScreen'); showWelcomePopup(un, _coins, true); toast('✅ Account ban gaya! 500🪙 mile!','success');
...
function nextQ() {
  ...
  quizState._timedOut = false; // [BUG-10 FIX] reset timeout flag per question
}
...
function checkAnswer() {
  ...
  // [BUG-10 FIX] Agar timer pehle se fire ho chuka hai to sessionTotal dobara mat badho
  if (!quizState._timedOut) quizState.sessionTotal++;
  ...
  if (ok) {
    ...
    if (quizState.sessionCorrect % 10 === 0) { toast(`🔥 ${quizState.sessionCorrect} sahi! +50🪙!`,'success',3000); const _su=getCurUser(); if(_su&&!_su.isAnonymous){updateDoc(doc(db,'users',_su.uid),{coins:increment(50)}).catch(()=>{});} /* [BUG-8 FIX] direct updateDoc bypasses rateLimiter */ }
  } else {
...
function _startTimer(sec) {
  ...
  timerManager.setInterval('quizTimer',()=>{
    ...
    if (t<=0) {
      ...
      quizState._timedOut = true; // [BUG-10 FIX] flag set karo — checkAnswer double count nahi karega
...
function startOnlineMatch(tableKey, matchId, initData) {
  ...
  // [BUG-5 FIX] feeDeducted=false hata diya — ab match start hone se refund eligibility nahi jaati
  // Refund sirf tab nahi milega jab match properly end ho (feeDeducted pehle hi clear hota hai _handleMatchEnd ke baad)
  MS.inMatchStarted = true;
  MS.myQIdx=0; MS.inSuddenDeath=false;
...
function _handleMatchEnd(won, reason='') {
  ...
  if (won&&coins>0) {
    const curUser=getCurUser();
    if (curUser&&!curUser.isAnonymous) {
      // [BUG-2 FIX] RTDB se match verify karke coins do — client-side exploit band
      const _verifyRef=ref(rtdb,`matches/${MS.tableKey}/${MS.matchId}`);
      get(_verifyRef).then(snap=>{
        const _d=snap.val();
        const _validWin = snap.exists() && _d.winner===MS.myRole && _d.status==='finished';
        if (_validWin||!snap.exists()) { // snap.exists()=false means already deleted (also valid)
          updateDoc(doc(db,'users',curUser.uid),{
            coins:increment(coins), totalWins:increment(1),
            totalGames:increment(1) // [BUG-6 FIX] match games bhi count mein
          }).catch(()=>{});
        }
      }).catch(()=>{
        // Fallback: agar RTDB read fail ho to bhi add karo (network issue)
        updateDoc(doc(db,'users',curUser.uid),{coins:increment(coins),totalWins:increment(1),totalGames:increment(1)}).catch(()=>{});
      });
      toast(`🏆 Jeet Gaye! +${coins}🪙`,'success',4000);
    }
  } else if (!won) {
    // [BUG-6 FIX] Haarne pe bhi totalGames increment karo
    const curUser=getCurUser();
    if (curUser&&!curUser.isAnonymous) {
      updateDoc(doc(db,'users',curUser.uid),{totalGames:increment(1)}).catch(()=>{});
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  on('exitMatchConfirm','click',()=>{
    hideExitConfirm();
    // [BUG-3 FIX] RTDB match document delete karo warna permanent leak hota hai
    if (MS.matchId && MS.tableKey && !MS._ended) {
      const _mRef = ref(rtdb,`matches/${MS.tableKey}/${MS.matchId}`);
      update(_mRef, {status:'abandoned', [`${MS.myRole}/connected`]:false}).catch(()=>{});
      timerManager.setTimeout(`deleteAbandon_${MS.matchId}`, ()=>remove(_mRef).catch(()=>{}), 5000);
    }
    leaveMatchCleanup(false); showScreen('welcomeScreen');
  });
