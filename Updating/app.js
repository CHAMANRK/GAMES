...
async function startGame() {
  ...
  selAyats=quranData.filter(a=>a.para>=fp&&a.para<=tp);
...
function checkAnswer() {
  ...
  const pn=parseInt(curAyat.page), ap=curAyat.para, aip=curAyat.pip;
  const maxPip = ap===29 ? 24 : ap===30 ? 25 : 20;
  let parts=[],opt=0;
  const pOk=parseInt(para)===ap; if(!pOk)parts.push(`❌ Para Galat! Sahi: ${ap}`);
  let pipOk=true;
  if(pip){
    const pipNum=parseInt(pip);
    if(pipNum<1||pipNum>maxPip){parts.push(`❌ PiP ${pipNum} valid nahi! Para ${ap} mein PiP 1-${maxPip} hota hai`);pipOk=false;}
    else{pipOk=pipNum===aip;if(!pipOk)parts.push(`❌ PiP Galat! Sahi: ${aip}`);}
  }
...
async function submitMatchAnswer(timeOut=false) {
  ...
  const pipOk= pip ? (parseInt(pip)>=1 && parseInt(pip)<=maxPip && parseInt(pip)===aip) : true;
