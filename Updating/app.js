...
// Script load ho gaya — HTML failsafe cancel karo
if (typeof window._clearBootFailsafe === 'function') window._clearBootFailsafe();
...
// FAILSAFE: Agar Firebase 8 sec mein respond na kare (slow network, timeout)
// toh boot loader khud hide ho jata hai — user auth screen pe pahunch jaata hai
const _bootFailsafe = setTimeout(() => {
  if (!_authResolved) {
    console.warn('⚠️ Boot failsafe triggered — Firebase ne 8s mein respond nahi kiya');
    hideBootLoader(true);
...
onAuthStateChanged(auth, async user => {
  ...
  if (user) {
    if (!user.isAnonymous) {
      try {
        // FIX: Firestore getDoc ko 5 sec timeout do — hang hone na de
        ...
        console.warn('User data load skip (timeout/error):', e.message);
        // Boot loader rok ke nahi rakhenge — aage badho
      }
...
async function startGame() {
  ...
  selAyats=quranData.filter(a=>{const p=a.para||(((a.page-1)/20|0)+1);return p>=fp&&p<=tp;}); // FIX: JSON pre-calculated para use karo
...
function nextQ() {
  ...
  const _maxPip=curAyat.para===29?24:curAyat.para===30?25:20;
  const _pipInp=$('user_page_in_para');if(_pipInp){_pipInp.max=_maxPip;_pipInp.placeholder=`PiP (1-${_maxPip})`;}
...
function checkAnswer() {
  ...
  const pn=parseInt(curAyat.page), ap=curAyat.para||(((pn-1)/20|0)+1), aip=curAyat.pip||(((pn-1)%20)+1); // FIX: JSON pre-calculated para/pip
  let parts=[],opt=0;
  const pOk=parseInt(para)===ap; if(!pOk)parts.push(`❌ Para Galat! Sahi: ${ap}`);
  let pipOk=true;
  if(pip){pipOk=parseInt(pip)===aip;if(!pipOk)parts.push(`❌ PiP Galat! Sahi: ${aip}`);}
  ...
  if(ok){
    ...
    if(mode==='survival'){
      ...
      if(sa){sa.innerHTML=`<b>Sahi Jawab:</b><br>Surah: <b>${curAyat.surah_name}</b> | Para: <b>${ap}</b> | Page: <b>${pn}</b> | PiP: <b>${aip}</b>`;sa.classList.remove('hidden');}
...
function showHint() {
  ...
  hints++;
  const hi = $('hintInfo');
  if (hi) hi.textContent = `Hint: ${hints}/${MAX_H}`;
  const hb = $('hintBtn');
  if (hb && hints >= MAX_H) hb.disabled = true;
...
// FIX: Pehle sirf diacritics hata rahe the — Alef variants alag rehte the
// Isliye "الحمد" search karne par "ٱلْحَمْدُ" nahi milta tha
// normalizeArabic: BOM + diacritics + alef variants sab ek saath normalize karta hai
function normalizeArabic(t) {
  if (!t) return '';
  t = String(t).replace(/\uFEFF/g, '');                           // BOM hatao
  t = t.replace(/[\u064B-\u065F\u0670\u06D6-\u06ED]/g, ''); // diacritics hatao
  t = t.replace(/[\u0622\u0623\u0624\u0625\u0626\u0671]/g, '\u0627'); // alef variants → ا
  t = t.replace(/\u0629/g, '\u0647');                            // taa marbuta → ha
  return t.trim().toLowerCase();
}
function rmD(t){ return normalizeArabic(t); } // backward compat
...
function doSearch(e){
  ...
  const found=quranData.filter(a=>{
    ...
    // FIX: JSON mein pre-calculated para use karo — formula galat tha
    if(String(a.para||((a.page-1)/20|0)+1)===raw.trim()) return true;
    return false;
  }).slice(0,30);

  if(!found.length){rd.innerHTML='<b>Koi result nahi mila.</b>';return;}

  const hl=t=>t.split(/(\s+)/).map(w=>normalizeArabic(w)===q?`<mark>${w}</mark>`:w).join('');
...
function freshMatchState() {
  return {
    ...
    feeDeducted: false,
    inSuddenDeath: false,
    _ended: false,         // FIX: explicitly defined taake double-end na ho
    _refunding: false,     // FIX: doRefund async lock — double refund rokne ke liye
...
async function openOnlineLobby() {
  ...
  loadQuran(); // background mein load karo

  showScreen('onlineLobbyScreen');

  // FIX: Fresh Firestore read — stale curData se coins galat dikhti thi
  try {
    const freshSnap = await getDoc(doc(db, 'users', curUser.uid));
    if (freshSnap.exists()) curData = freshSnap.data();
  } catch(e) { /* stale curData use karo */ }
...
// ── Join table ──
let _joining = false; // FIX: Double-click guard — ek baar se zyada joinTable na chale
async function joinTable(tableKey) {
  ...
  if (_joining)             { toast('⏳ Thoda ruko...','info'); return; } // FIX: Double-click guard
  _joining = true;

  // FIX: Agar pehle se koi match chal raha tha (ghost state), usse clean karo
  // Yahi "table click se match cancel" wala bug tha — MS reset hota tha par listeners rehte the
  ...
  // FIX: Fresh coins read karo — lobby se pehle already fresh read ho chuka hai
  // lekin double-click ya race condition ke liye dobara verify karo
  try {
    const freshSnap = await getDoc(doc(db, 'users', curUser.uid));
    if (freshSnap.exists()) curData = freshSnap.data();
  } catch(e) {}
  const coins = curData?.coins || 0;
  if (coins < table.fee) { _joining = false; toast(`❌ ${table.fee}🪙 chahiye! Aapke paas: ${coins}🪙`,'error'); return; }

  if (!quranData.length) {
    toast('⏳ Data load ho raha hai...','info');
    await loadQuran();
    if (!quranData.length) { _joining = false; toast('❌ Quran data load nahi hua!','error'); return; }
  }
  ...
  try {
    ...
    _joining = false; // FIX: Lock hamesha release ho — chahe error ho ya nahi
  }
}
...
async function findOrCreateMatch(tableKey) {
  ...
  if (asP2) {
    ...
    // FIX: Queue removal BAAD mein — p2JoinMatch ke andar match 'active' hone ke baad
    // Pehle yahan remove() hota tha jisse P2 crash ho toh P1 hang ho jaata tha
    MS.matchId = p2MatchId;
    MS.myRole  = 'p2';
    MS.opponentName = oppData?.username || 'Player';
    await p2JoinMatch(p2MatchId, tableKey, qRef); // qRef pass karo taake andar hata sake
    ...
    MS.autoCancel = setTimeout(async () => {
      try {
        ...
        if (snap.exists() && snap.val().status === 'waiting') {
          // Waiting match delete karo — koi nahi aaya
          await remove(ref(rtdb, `matches/${tableKey}/${matchId}`)).catch(()=>{});
          await remove(ref(rtdb, `queues/${tableKey}`)).catch(()=>{});
          MS.matchId = null; MS.myRole = null; // leaveMatchCleanup ko match delete karne se rokna
          leaveMatchCleanup(false);
          await doRefund();
...
function p1WaitForOpponent(tableKey, matchId) {
  ...
  // FIX: callback ko store karo taake off(mRef, callback) properly kaam kare
  const callback = snap => {
    if (!snap.exists()) return;
    const d = snap.val();
    if (d.status === 'active' && d.p2?.uid) {
      off(mRef, callback); // specific listener hatao
      MS.opponentName = d.p2.name;
      startOnlineMatch(tableKey, matchId, d);
    }
  };
  onValue(mRef, callback, err => console.warn('P1 wait error:', err.message));
  MS.listeners.push({ ref: mRef, fn: callback });
}
...
async function p2JoinMatch(matchId, tableKey, qRef) {
  ...
  // FIX: Queue SIRF tab hatao jab match successfully 'active' ho gaya
  // Pehle yeh pehle hota tha — agar P2 crash karta toh P1 hang ho jaata
  if (qRef) { try { await remove(qRef); } catch(e) {} }
...
function startOnlineMatch(tableKey, matchId, initData) {
  ...
  const unsub = onValue(mRef, snap => {
    if (!snap.exists()) {
      // FIX: Agar match already properly end hua hai toh ignore karo
      // (P1 ne 8 sec baad cleanup kiya — yeh expected hai)
      if (MS._ended) return;
      // Genuinely unexpected deletion — opponent left ya admin cleared
    ...
    // ── Opponent disconnect/quit detection ──
    const oppConnected = opD?.connected;
    if (oppConnected === false && !MS.disconnectGraceTimer && (d.status === 'active' || d.status === 'sudden_death')) {
      // FIX: sudden_death mein bhi grace timer chalna chahiye — pehle sirf 'active' check tha
      const GRACE_SEC = 15;
      ...
      MS.disconnectGraceTimer = setTimeout(async () => {
        try {
          ...
          if (freshOpD?.connected === false && currD.status !== 'finished') {
            // FIX: Sirf winner likhein agar kisi ne pehle nahi likha
            // runTransaction use karo taake dono player ek saath na likhen
            const winRef = ref(rtdb, `matches/${tableKey}/${matchId}/winner`);
            try {
              await runTransaction(winRef, current => {
                if (current && current !== '') return; // already set
                return MS.myRole; // main hi jeetha hoon
              });
              await update(mRef, { status: 'finished' });
            } catch(e) { console.warn('Auto-win tx err:', e); }
            // Ab handleMatchEnd chalao — coins bhi wahan milenge
      ...
      // Opponent wapas aa gaya — grace cancel karo
    ...
    // FIX: Apni question hum advanceMyQuestion() se seedha dikhate hain
    // Listener SIRF opponent sync aur match-end conditions ke liye hai
    // Agar MS.myQIdx listener se peeche reh gaya ho (reconnect/refresh case) toh sync karo
    const newMyQIdx = myD?.qIdx || 0;
    if (newMyQIdx > MS.myQIdx) {
      // Reconnect/stale state — silently sync karo, question mat dikhao
      MS.myQIdx = newMyQIdx;
  ...
  // FIX: unsub function store karo — off(ref) ke bajaye proper cleanup hogi
  MS.listeners.push({ ref: mRef, unsub });
}
...
function showMatchQuestion(ayatIndex, qNum, totalQuestions, isSuddenDeath = false) {
  ...
  // Dynamic maxPip for match mode
  const _mMaxPip=ayat.para===29?24:ayat.para===30?25:20;
  const _mPipInp=$('match_pip');if(_mPipInp){_mPipInp.max=_mMaxPip;_mPipInp.placeholder=`PiP (1-${_mMaxPip})`;}
...
function startMatchTimer(sec) {
  ...
  MS.matchTimerInterval = setInterval(() => {
    ...
    if(t<=0){
      ...
      MS.matchTimerInterval = null; // FIX: null set karo — stale interval ID na rahe
...
async function submitMatchAnswer(timeOut=false) {
  ...
  const ap   = ayat.para||(((pn-1)/20|0)+1);   // FIX: JSON pre-calculated para
  const aip  = ayat.pip ||(((pn-1)%20)+1);     // FIX: JSON pre-calculated pip
  ...
  if (pOk && pipOk) {
    ...
    // FIX: Race condition — MS.myScore stale ho sakta tha agar listener lag kare
    // runTransaction se atomic increment hota hai — sahi score hamesha jaata hai RTDB mein
...
async function advanceMyQuestion(wasCorrect) {
  await sleep(1400);
  if (MS._ended) return; // match already over
  ...
  // FIX: LOCAL state pehle update karo — listener sync pe depend mat karo
  MS.myQIdx = newIdx;

  if (!MS.inSuddenDeath && newIdx >= tbl.totalQ) {
    // Apne saare sawaal khatam — RTDB update karo, wait screen dikhao
    try { await update(mRef, { [qIdxField]: newIdx }); } catch(e) {}
    // Opponent ka intezaar — aur sawaal nahi dikhana
    showWaitingForOpponent();
    return;
  }

  // RTDB update karo aur SEEDHA next question dikhao (listener pe depend mat karo)
  try { await update(mRef, { [qIdxField]: newIdx }); } catch(e) { console.warn('AdvanceMyQ err:', e); }

  // FIX: Direct question show karo — yahi main bug tha (next question nahi aata tha)
  // Listener sirf opponent ke liye sync karta hai, apni question hum khud dikhate hain
...
// Apne saare sawaal khatam — opponent ka wait karo
function showWaitingForOpponent() {
  const tbl = TABLES[MS.tableKey] || {};
  // Timer aur input band karo
  clearInterval(MS.matchTimerInterval);
  const cb = $('matchCheckBtn'); if (cb) cb.disabled = true;
  const fill = $('matchTimerFill'); if (fill) fill.style.width = '0%';
  const txt  = $('matchTimer');    if (txt)  txt.textContent  = '⏳';
  // Progress bar mein wait message
  const mp = $('matchProgress');
  if (mp) mp.textContent = `✅ Aapne ${tbl.totalQ || ''} sawaal khatam kiye — ${MS.opponentName || 'Opponent'} ka intezaar...`;
  // Ayat area mein bhi wait dikhao
  const at = $('matchAyatText');
  if (at) at.textContent = '⏳ Opponent abhi khel raha hai...';
  // Result area clear karo
  const mr = $('matchResult'); if (mr) { mr.classList.add('hidden'); mr.textContent = ''; }
  // Overlay popup bhi dikhao
...
function handleMatchEnd(won, reason='') {
  ...
  // FIX: Match node turant delete mat karo — dono players ko result dekhna hai
  // 8 second baad P1 delete kare, P2 sirf connected:false kare
...
function leaveMatchCleanup(refund=false) {
  ...
  MS.autoCancel = null;
  MS.disconnectGraceTimer = null;
  MS.listeners.forEach(l=>{ try{ if(l && l.unsub) { l.unsub(); } else if(l && l.ref) { if(l.fn) off(l.ref,l.fn); else off(l.ref); } }catch(e){} });
  MS.listeners = [];
  if (MS.tableKey) {
    // Sirf apni queue entry hatao — pehle check karo ke wahan apna uid hai
    ...
    // FIX: Sirf tab connected:false karo jab match normally chhod rahe hain
    // Agar match properly end hua (_ended=true) toh yeh mat karo — opponent ko wrong auto-win milega
...
async function doRefund() {
  // FIX: _refunding lock — agar pehle se chal rahi hai toh dobara mat chalao
...
// ── Opponent Wait Popup ──
// allDone=true: aapne SARE sawaal khatam kar liye, opponent abhi khel raha hai
// allDone=false: ek question ke baad brief wait (legacy, ab use nahi hota)
...
function showDisconnectGracePopup(seconds = 5) {
  ...
  // FIX: Agar pehle se koi interval chal raha tha (multiple calls), usse clear karo
  if (popup._interval) { clearInterval(popup._interval); popup._interval = null; }
  popup.innerHTML = `
  ...
  try {
    ...
    if(!friends.length){
      ...
      listEl.innerHTML=fDocs.map((fd,i)=>{
        ...
        const d=fd.data();
        // FIX: onclick mein username seedha mat daalo (XSS risk agar username mein quotes hon)
        // data-uid aur data-name use karo — event listener neeche handle karta hai
      ...
      // FIX: Event delegation — har button ke liye alag listener nahi, ek kaam karta hai
      listEl.querySelectorAll('.unfriend-btn').forEach(btn => {
        btn.addEventListener('click', () => unfriendUser(btn.dataset.uid, btn.dataset.name));
      });
    }
    ...
    if(!pending.length){
      ...
      if(pendEl)pendEl.innerHTML=pDocs.map((pd,i)=>{
        ...
        const d=pd.data();
        // FIX: data-uid/data-name use karo — onclick mein username seedha nahi
        return`<div class="friend-card">
          <div class="friend-avatar">👤</div>
          <div class="friend-info">
            <div class="friend-name">${esc(d.username||'Player')}</div>
      ...
      // FIX: Event listeners — safe tarika
      if(pendEl) {
        pendEl.querySelectorAll('.accept-btn').forEach(btn => {
          btn.addEventListener('click', () => acceptFriend(btn.dataset.uid, btn.dataset.name));
        });
        pendEl.querySelectorAll('.reject-btn').forEach(btn => {
          btn.addEventListener('click', () => rejectFriend(btn.dataset.uid));
        });
      }
    }
  } catch(e){
...
async function searchUserByUID() {
  ...
  try {
    ...
    if(preview){
      ...
      // FIX: onclick mein uid/name seedha nahi — event listener safely attach karo
      const addBtn = preview.querySelector('.add-friend-preview-btn');
      if (addBtn) {
        addBtn.addEventListener('click', () => sendFriendRequest(addBtn.dataset.uid, addBtn.dataset.name));
      }
    }
  } catch(e){
...
on('cancelMatchBtn','click', async ()=>{
  // Agar waiting match hai toh delete karo
