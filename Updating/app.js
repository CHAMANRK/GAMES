...
async function joinTable(tableKey) {
  ...
  // FIX: Fresh Firestore read karo taake stale curData se galat coins na dikhen
  let freshCoins = curData.coins || 0;
  try {
    const freshSnap = await getDoc(doc(db,'users',curUser.uid));
    if (freshSnap.exists()) { freshCoins = freshSnap.data().coins || 0; curData = freshSnap.data(); }
  } catch(e) { /* stale value se hi kaam chalao */ }

  if (freshCoins < table.fee) { toast(`❌ ${table.fee}🪙 chahiye!`,'error'); return; }
...
function startOnlineMatch(tableKey, matchId, initData) {
  ...
  const unsub = onValue(mRef, snap => {
    ...
    if (oppConnected === false && !MS.disconnectGraceTimer && d.status === 'active') {
      const GRACE_SEC = 15;
    ...
    if (d.winner && d.status === 'finished') {
      off(mRef); // FIX: listener band karo taake dobara fire na ho
      handleMatchEnd(d.winner === MS.myRole);
      return;
    }

    // ── Win condition: first to firstTo ──
    if (MS.myScore  >= tbl.firstTo) { off(mRef); handleMatchEnd(true);  return; }
    if (MS.oppScore >= tbl.firstTo) { off(mRef); handleMatchEnd(false); return; }
    ...
    if (newMyQIdx !== MS.myQIdx && d.questionPool && d.questionPool[newMyQIdx] !== undefined) {
      ...
      // FIX: Agar mujhe totalQ sawaal ho gaye hain toh nayi question mat dikhao
      // Yeh tab hota hai jab RTDB sync se listener dobara fire kare
      if (!MS.inSuddenDeath && newMyQIdx < tbl.totalQ) {
...
async function advanceMyQuestion(wasCorrect) {
  ...
  MS.myQIdx = newIdx; // FIX: local state pehle update karo taake listener mein duplicate question na aaye

  if (!MS.inSuddenDeath && newIdx >= tbl.totalQ) {
    // FIX: Mujhe aur sawaal nahi dikhane — opponent ka wait karo
    try { await update(mRef, { [qIdxField]: newIdx }); } catch(e) {}
    // Opponent ka intezaar popup dikhao — input/timer band karo
    showOpponentWaitPopup(true); // true = sare sawaal khatam
    const cb=$('matchCheckBtn'); if(cb) cb.disabled=true;
    clearInterval(MS.matchTimerInterval);
    const fill=$('matchTimerFill'); if(fill) fill.style.width='0%';
    const txt=$('matchTimer'); if(txt) txt.textContent='\u23f3';
    const mp=$('matchProgress'); if(mp) mp.textContent=`✅ Aapne ${tbl.totalQ} sawaal khatam kiye — Opponent ka intezaar...`;
    return;
  }
...
function leaveMatchCleanup(refund=false) {
  ...
  if (MS.tableKey) {
    ...
    // Match node bilkul delete mat karo — opponent ko pata chalna chahiye
    // Sirf apna connected:false karo taake opponent auto-win le sake
...
// ── Opponent Wait Popup ──
// FIX: Ab yeh do cases handle karta hai:
//   1. Aapne apne saare sawaal khatam kar liye — opponent still playing
//   2. Legacy: opponent next sawaal submit kar raha hai (intermediate)
function showOpponentWaitPopup(waitingForAll = false) {
  let popup = $('oppWaitPopup');
  if (!popup) {
    popup = document.createElement('div');
    popup.id = 'oppWaitPopup';
    popup.style.cssText = 'position:fixed;inset:0;background:rgba(5,15,10,.92);backdrop-filter:blur(10px);z-index:8000;display:flex;align-items:center;justify-content:center;';
    document.body.appendChild(popup);
  }
  const tbl = TABLES[MS.tableKey] || {};
  if (waitingForAll) {
    popup.innerHTML = `<div style="background:var(--bg-card);border:1px solid #00c47228;border-radius:20px;padding:32px 24px;text-align:center;max-width:320px;width:90vw;display:flex;flex-direction:column;gap:16px">
      <div style="font-size:2.5rem">✅</div>
      <div style="font-family:Cinzel,serif;font-size:1.1rem;color:var(--gold-light)">Aapne Sawaal Khatam Kiye!</div>
      <div style="font-size:1.6rem;font-weight:900;color:var(--emerald)">${MS.myScore} <span style="color:var(--text-muted);font-size:1rem">points</span></div>
      <div style="font-size:0.88rem;color:var(--text-muted);font-family:Tajawal,sans-serif">
        ${MS.opponentName||'Opponent'} abhi bhi khel raha hai...<br>
        <span style="color:var(--gold);font-size:0.82rem">Nateeja unke khatam hone ke baad aayega</span>
      </div>
